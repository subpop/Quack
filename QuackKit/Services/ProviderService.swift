...
@Observable
@MainActor
public final class ProviderService: ProviderServiceProtocol {
    ...
    public var mlxModelService: (any MLXModelProviding)?
    ...
    public func resolvedProfile(for session: ChatSession, profiles: [ProviderProfile]) -> ProviderProfile? {
    ...
    public func resolvedModel(for session: ChatSession, profiles: [ProviderProfile]) -> String {
    ...
    public func fallbackProfile(from profiles: [ProviderProfile]) -> ProviderProfile? {
    ...
    public func makeClient(
        ...
        // For MLX, resolve the pre-loaded container (type-erased to avoid
        // importing MLXLMCommon in this file).
        let mlxContainer: Any?
        if profile.platform == .mlx {
            mlxContainer = mlxModelService?.cachedContainerAsAny(for: model)
        } else {
            mlxContainer = nil
        }
    ...
    public var mlxLoadState: MLXLoadState {
        mlxModelService?.loadState ?? .idle
    }
