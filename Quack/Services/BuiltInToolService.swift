...
final class BuiltInToolService {
    ...
    var enabledToolSummaries: [ToolSummary] {
        BuiltInTool.availableCases.compactMap { tool in
            ...
            return ToolSummary(builtInTool: tool, name: instance.name, description: instance.description)
        }
    }

    struct ToolSummary: Identifiable {
        let builtInTool: BuiltInTool
        let name: String
    ...
    private func save() {
        ...
        do {
            ...
            Logger.database.error("Failed to save built-in tool settings: \(error)")
        }
    }
    ...
    private func load() {
        ...
        do {
            ...
            Logger.database.error("Failed to load built-in tool settings: \(error)")
        }
    }
}
